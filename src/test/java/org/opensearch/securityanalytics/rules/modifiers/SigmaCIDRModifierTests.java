...
public class SigmaCIDRModifierTests extends SigmaModifierTests {
    ...
    public void testCidrPreservesHostBits() throws SigmaRegularExpressionError, SigmaValueError, SigmaModifierError {
        ...
        Assert.assertTrue(values.get(0) instanceof SigmaCIDRExpression);
        Assert.assertEquals("192.168.1.5/24", ((SigmaCIDRExpression) values.get(0)).getCidr());

        values = new SigmaCIDRModifier(dummyDetectionItem(), Collections.emptyList()).apply(Either.left(new SigmaString("192.168.1.31/27")));
        Assert.assertEquals("192.168.1.31/27", ((SigmaCIDRExpression) values.get(0)).getCidr());
    }

    public void testCidrPreservesIPv6Form() throws SigmaRegularExpressionError, SigmaValueError, SigmaModifierError {
        ...
        Assert.assertTrue(values.get(0) instanceof SigmaCIDRExpression);
        Assert.assertEquals("2001:db8::1/32", ((SigmaCIDRExpression) values.get(0)).getCidr());
    }
